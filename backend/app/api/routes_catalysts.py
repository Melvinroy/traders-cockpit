...
def _row_payload(row: dict[str, Any]) -> dict[str, Any]:
    ...
    generated_at = payload.get("generated_at_sgt")
    if generated_at is not None:
        payload["generated_at_sgt"] = generated_at.isoformat()
...
def build_router() -> APIRouter:
    ...
    def get_catalysts(
        ...
        # The production Postgres database contains the read-only
        # catalyst_dashboard_rows view. Local SQLite development may not;
        # return a valid empty dashboard instead of breaking the Journal.
        try:
            as_of = db.execute(
            ...
            rows = db.execute(
                text(
                    ...
                    with windowed as (
                        select *,
                               count(*) over (partition by ticker) as appearances
                        from catalyst_dashboard_rows
                        where trading_date_checked between (:as_of::date - (:days - 1)) and :as_of::date
                    ), latest as (
